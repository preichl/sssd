...
static void dp_req_reply_error(struct dp_sbus_req_item *list,
                               const char *req_name,
                               errno_t ret)
{
    ...
    case EOK:
        ...
        error = sbus_error_new(NULL, SBUS_ERROR_INTERNAL,
    ...
    case ERR_OFFLINE:
        ...
        error = sbus_error_new(NULL, SBUS_ERROR_DP_OFFLINE,
    ...
    default:
        ...
        error = sbus_error_new(NULL, SBUS_ERROR_DP_FATAL,
...
static void dp_req_reply_success(struct dp_sbus_req_item *list,
                        ...
                        const char *output_dtype)
{
    ...
    if (request_key == NULL) {
        ...
        request_key = talloc_asprintf(tmp_ctx, "%p", sbus_req);
        if (request_key == NULL) {
            ret = ENOMEM;
            goto done;
        }
    ...
    key = dp_req_table_key(tmp_ctx, target, method, dp_flags, request_key);
    if (key == NULL) {
        ret = ENOMEM;
        goto done;
    }
    ...
    ret = dp_req_with_reply_step(provider, dp_cli, domain, request_name, key,
                                 ...
                                 request_data, reply_fn, output_dtype);
                                      ...
                                      const char *output_dtype)
{
    ...
    state->key = talloc_strdup(state, request_key);
    if (state->key == NULL) {
        ret = ENOMEM;
        goto done;
    }
...
static void dp_req_with_reply_done(struct tevent_req *req)
{
    ...
    ret = dp_req_recv(state, req, state->output_dtype, &output_data);
    if (ret != EOK) {
        dp_req_reply_error(value->list, state->name, ret);
        goto done;
    }

    /* reply with data */
    dp_req_reply_success(value->list, state->reply_fn,
                         state->name, output_data);

done:
    /* This order is important since state is attached to req. */
    dp_req_table_del(state->provider->requests.reply_table, state->key);
    talloc_free(value);
    talloc_free(req);
}
