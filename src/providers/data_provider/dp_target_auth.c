...
static void dp_pam_reply(const char *request_name,
                               ...
                               struct pam_data **_pd)
{
    ...
    ret = dp_unpack_pam_request(sbus_req->message, mem_ctx, &pd, &dbus_error);
    if (ret != EOK) {
...
errno_t dp_pam_handler(struct sbus_request *sbus_req, void *sbus_data)
{
    ...
    if (target == DP_TARGET_SENTINEL) {
        ...
        dp_pam_reply(req_name, sbus_req, pd);
        return EOK;
    }
    ...
    tevent_req_set_callback(req, dp_pam_handler_step_done, pd);
                                  ...
                                  int dp_error)
{
    ...
    if (!dp_target_enabled(provider, NULL, DPT_SELINUX)) {
        return false;
    }
...
void dp_pam_handler_step_done(struct tevent_req *req)
{
    ...
    ret = dp_req_recv(pd, req, struct dp_reply_std, &reply);
    talloc_zfree(req);
    if (ret != EOK) {
    ...
    if (!should_invoke_selinux(state->provider, pd, reply->dp_error)) {
        /* State and request related data are freed with sbus_req. */
        dp_pam_reply(state->request_name, state->sbus_req, pd);
        return;
    }
