                ...
                struct dp_req **_dp_req)
{
    ...
    if (domainname != NULL) {
        domain = find_domain_by_name(be_ctx->domain, domainname, true);
        if (domain == NULL) {
                               ...
                               const char **_request_name)
{
    ...
    /* Make sure that *_request_name is not NULL if error happens from this
     * point forward so the callers do not need act upon it. */
    *_request_name = "Request Not Yet Created";

    if (!dp_target_initialized(provider->targets, target)) {
        ret = ERR_MISSING_DP_TARGET;
        goto immediately;
    }
