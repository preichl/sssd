    ...
    if (fn != NULL) {
        ...
        if (error == NULL) {
            ...
            DEBUG(SSSDBG_FATAL_FAILURE, "Unable to load module [%s] "
                  "constructor: %s", module->name, error);
            ret = ELIBBAD;
            goto done;
        }
    }
...
static int dp_module_destructor(struct dp_module *module)
{
    if (module->libhandle != NULL) {
        /* TODO generates SIGSEGV -- why? */
                                          ...
                                          const char *name)
{
    ...
    talloc_set_destructor(module, dp_module_destructor);
