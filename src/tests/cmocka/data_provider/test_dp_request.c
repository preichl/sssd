...
static void test_get_name_by_uid(void **state)
{
    ...
    ret = dp_req_recv(test_ctx, req, struct recv_data, (void**)&recv_data);
    assert_int_equal(ret, EOK);
    assert_string_equal(recv_data->name, NAME);
    talloc_free(recv_data);

    ret = dp_req_recv(test_ctx, req2, struct recv_data, (void**)&recv_data);
    assert_int_equal(ret, ENOENT);

    ret = dp_req_recv(test_ctx, req3, struct recv_data, (void**)&recv_data);
    assert_int_equal(ret, EOK);
